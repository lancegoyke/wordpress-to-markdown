  ...
  const payloads = posts.flatMap((post) => {
    ...
    if (checkFile(destinationPath)) {
      ...
      const payload = {
        item: post,
        name:
          (!config.includeOtherTypes
            ? ""
            : post.meta.type === "post"
            ? "blog" + " - "
            : post.meta.type + " - ") + post.meta.slug,
        destinationPath,
        delay,
      };
...
async function writeImageFilesPromise(posts, config) {
  ...
  const payloads = posts.flatMap((post) => {
    ...
    if (config.postFolders) {
      // store images alongside the post
      const postPath = getPostPath(post, config);
      imagesDir = path.join(path.dirname(postPath), "images");
    } else {
      // store images in the assets folder
      imagesDir = path.join(config.output, "assets/images");
    }
